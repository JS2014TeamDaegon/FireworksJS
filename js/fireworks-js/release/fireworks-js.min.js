...
var timerTick=fireworks.options.timerTick;fireworks.init=function loop(options){if(options!=null){fireworks.options=new fireworks.Options(options);}
...
if(timerTick>=fireworks.options.timerTotal){fireworks.fireworks.push(new fireworks.Firework(fireworks.canvasWidth/2,fireworks.canvasHeight,fireworks.random(0,fireworks.canvasWidth),fireworks.random(0,fireworks.canvasHeight/2)));timerTick=fireworks.options.timerTick;}else{timerTick++;}}
window.onresize=function(){setWidthAndHeight();};function setWidthAndHeight(){var docElement=window.document.documentElement,body=window.document.getElementsByTagName('body')[0],width=window.innerWidth||docElement.clientWidth||body.clientWidth,height=window.innerHeight||docElement.clientHeight||body.clientHeight;if(fireworks.options!==undefined){fireworks.options.width=width;fireworks.options.height=height;}
