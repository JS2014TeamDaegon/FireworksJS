...
Firework.prototype.draw=function(){fw.ctx.beginPath();fw.ctx.moveTo(this.coordinates[this.coordinates.length-1][0],this.coordinates[this.coordinates.length-1][1]);fw.ctx.lineTo(this.x,this.y);fw.ctx.strokeStyle='hsl('+fw.hue+',100%,'+this.brightness+'%)';fw.ctx.stroke();fw.ctx.beginPath();fw.ctx.arc(this.tx,this.ty,this.targetRadius,0,360*Math.PI/180);fw.ctx.stroke();}})(window.Fireworks=window.Fireworks||{});﻿(function(fw,window){window.requestAnimationFrame=(function(){return window.requestAnimationFrame||window.webkitRequestAnimationFrame||window.mozRequestAnimationFrame||function(callback){window.setTimeout(callback,1000/60);};})();fw.canvas=document.getElementById('fireworks-field');fw.iframe=document.getElementById('fireworks-target');(function setWidthHeight(){var w=window,d=document,e=d.documentElement,g=d.getElementsByTagName('body')[0],x=w.innerWidth||e.clientWidth||g.clientWidth,y=w.innerHeight||e.clientHeight||g.clientHeight;fw.canvas.width=x;fw.canvas.height=y;})();fw.ctx=fw.canvas.getContext('2d');fw.cw=fw.canvas.clientWidth;fw.ch=fw.canvas.clientHeight;fw.fireworks=[];fw.particles=[];fw.hue=180;fw.random=function random(min,max){return Math.random()*(max-min)+min;};fw.calculateDistance=function calculateDistance(p1x,p1y,p2x,p2y){var xD=p1x-p2x;var yD=p1y-p2y;return Math.sqrt(Math.pow(xD,2)+Math.pow(yD,2))}
...
var limiterTotal=5;var limiterTick=0;var timerTotal=100;var timerTick=0;var mx;var my;var mousedown=false;fw.init=function loop(address){if(address!=null){fw.iframe.src=address;}
window.requestAnimationFrame(loop.bind(null,null));fw.hue+=0.5;fw.ctx.globalCompositeOperation='destination-out';fw.ctx.fillStyle='rgba(0,0,0,0.4)';fw.ctx.fillRect(0,0,fw.cw,fw.ch);fw.ctx.globalCompositeOperation='lighter'
