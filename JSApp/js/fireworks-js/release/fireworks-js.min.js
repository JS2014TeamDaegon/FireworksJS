...
function getRandomColor(){var red=Math.floor(Math.random()*256),green=Math.floor(Math.random()*256),blue=Math.floor(Math.random()*256);return'rgb('+red+','+green+','+blue+')';}})(window.Fireworks=window.Fireworks||{});﻿(function(fireworks,window){window.requestAnimationFrame=(function(){return window.requestAnimationFrame||window.webkitRequestAnimationFrame||window.mozRequestAnimationFrame||function(callback){window.setTimeout(callback,1000/60);};})();var docElement=window.document.documentElement,body=window.document.getElementsByTagName('body')[0],width=window.innerWidth||docElement.clientWidth||body.clientWidth,height=window.innerHeight||docElement.clientHeight||body.clientHeight;fireworks.canvas=document.createElement("canvas");fireworks.canvas.id="fireworks-field";setPositionAndAppendToDocument(fireworks.canvas);fireworks.canvas.width=width;fireworks.canvas.height=height;fireworks.svg=document.createElementNS("http://www.w3.org/2000/svg","svg");fireworks.svg.id="the-svg";setPositionAndAppendToDocument(fireworks.svg);fireworks.svg.style.width=width;fireworks.svg.style.height=height;fireworks.ctx=fireworks.canvas.getContext('2d');fireworks.canvasWidth=fireworks.canvas.clientWidth;fireworks.canvasHeight=fireworks.canvas.clientHeight;fireworks.options=new fireworks.Options();fireworks.fireworks=[];fireworks.particles=[];fireworks.random=function random(min,max){return Math.random()*(max-min)+min;};fireworks.calculateDistance=function calculateDistance(point1X,point1Y,point2X,point2Y){var distanceX=point1X-point2X;var distanceY=point1Y-point2Y;return Math.sqrt(Math.pow(distanceX,2)+Math.pow(distanceY,2))}
