...
Particle.prototype.update=function(index){this.coordinates.pop();this.coordinates.unshift([this.x,this.y]);this.speed*=this.friction;this.x+=Math.cos(this.angle)*this.speed;this.y+=Math.sin(this.angle)*this.speed+this.gravity;this.alpha-=this.decay;if(this.alpha<=this.decay){fireworks.particles.splice(index,1);}};Particle.prototype.draw=function(){fireworks.ctx.beginPath();fireworks.ctx.moveTo(this.coordinates[this.coordinates.length-1][0],this.coordinates[this.coordinates.length-1][1]);fireworks.ctx.lineTo(this.x,this.y);fireworks.ctx.strokeStyle='hsla('+this.hue+', 100%, '+this.brightness+'%, '+this.alpha+')';fireworks.ctx.stroke();};})(window.Fireworks=window.Fireworks||{});﻿(function(fw){fw.greeting=greeting;function greeting(){var userScreenWidth=fw.canvasWidth;var userScreenHeight=fw.canvasHeight;var textInput=document.getElementById('heading').value;var wishes=document.getElementById('wishes').value;var wishesColor="#d1fe01";var svgNameSpace='http://www.w3.org/2000/svg';var svg=document.getElementById('the-svg');svg.innerHTML="";var textFont="Verdana";var textRowWidth=10;var textFontSize=34;var update=2;var textSpeed=30;var animationCounter=100;var MAX_FONT_SIZE=72;var MIN_FONT_SIZE=32;var text;var greetings;text=drawText(userScreenWidth/8-textFontSize,userScreenHeight/2.5,textInput,"#aaaf18",textFontSize,"bold",textFont);svg.appendChild(text);greetings=drawText(userScreenWidth/8-textFontSize,userScreenHeight/2+textFontSize,wishes,wishesColor,textFontSize,"bold","Verdana");svg.appendChild(greetings);function createMultilineText(output,inputText,coordinateX,coordinateY,size){var words=inputText.split(' ');var word=words.shift();var newTSElement=document.createElementNS(svgNameSpace,"tspan");var textNode=document.createTextNode(word+" ");var tSpanElement=newTSElement.cloneNode(true);tSpanElement.setAttribute('x',coordinateX);tSpanElement.setAttribute('y',coordinateY);tSpanElement.appendChild(textNode);output.appendChild(tSpanElement);coordinateY+=textFontSize;while(words.length>0){word=words.shift();console.log(textNode.data.length);textNode=document.createTextNode(word+" ");if(tSpanElement.innerHTML.length+word.length+size/20>userScreenWidth/35){tSpanElement.setAttribute('x',coordinateX);tSpanElement.setAttribute('y',coordinateY);coordinateY+=textFontSize;output.appendChild(tSpanElement);tSpanElement=newTSElement.cloneNode(true);}
tSpanElement.appendChild(textNode);}
output.appendChild(tSpanElement);}
function drawText(coordinateX,coordinateY,input,fill,fontSize,fontWeight,fontFamily,stroke,sWidth){var text,strokeColor=stroke||'black',strokeWidth=sWidth||3;text=document.createElementNS(svgNameSpace,'text');createMultilineText(text,input,coordinateX,coordinateY,fontSize);text.setAttribute('x',coordinateX);text.setAttribute('y',coordinateY);text.setAttribute('fill',fill);text.setAttribute('font-size',fontSize);text.setAttribute('font-weight',fontWeight);text.setAttribute('font-family',fontFamily);text.setAttribute('stroke',strokeColor);text.setAttribute('stroke-width',strokeWidth);return text;}
function updateText(){if(animationCounter>0){if(update>0){if(textFontSize<MAX_FONT_SIZE){textFontSize+=update;}
else{update=-update;}}
else{if(textFontSize>MIN_FONT_SIZE){textFontSize+=update;}
else{update=-update;}}
text.parentNode.removeChild(text);animationCounter--;text=drawText(userScreenWidth/8-textFontSize,userScreenHeight/2.5,textInput,getRandomColor(),textFontSize,"bold",textFont);svg.appendChild(text);}
else{text.innerHTML="";greetings.innerHTML="";}}
setInterval(function(){updateText();},textSpeed);}
function getRandomColor(){var red=Math.floor(Math.random()*256),green=Math.floor(Math.random()*256),blue=Math.floor(Math.random()*256);return'rgb('+red+','+green+','+blue+')';}})(window.Fireworks=window.Fireworks||{});﻿(function(fireworks){fireworks.Options=Options;function Options(options){if(options===undefined){options={};}
...
if(timerTick>=fireworks.options.timerTotal){fireworks.fireworks.push(new fireworks.Firework(fireworks.canvasWidth/2,fireworks.canvasHeight,fireworks.random(0,fireworks.canvasWidth),fireworks.random(0,fireworks.canvasHeight/2)));timerTick=0;}else{timerTick++;}};window.onresize=function(){setWidthAndHeight();};function setWidthAndHeight(){var docElement=window.document.documentElement,body=window.document.getElementsByTagName('body')[0],width=window.innerWidth||docElement.clientWidth||body.clientWidth,height=window.innerHeight||docElement.clientHeight||body.clientHeight;if(fireworks.options!==undefined){fireworks.options.width=width;fireworks.options.height=height;}
